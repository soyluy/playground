...
export class TodoService {
  private readonly _persistenceService = inject(TodoPersistenceService);
	...
	private readonly _activeFilter: Signal<TodoFilter | null> = this._filterService.getFilter();
  ...
  constructor() {
    const todos$ = toObservable(computed(()  => ({
			filter: this._activeFilter(),
			_refresh: this._refresh(),
		}))).pipe(
			debounceTime(300),
			switchMap(({filter}) => this._persistenceService.getTodos(filter ?? undefined)),
		);
