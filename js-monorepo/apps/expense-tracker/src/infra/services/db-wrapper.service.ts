...
export class DbWrapperService {
  ...
  constructor() {
    this.ready = new Promise((resolve, reject) => {
      const req = indexedDB.open(environment.databaseName, environment.databaseVersion);
      ...
      req.onupgradeneeded = (event: IDBVersionChangeEvent) => {
        ...
        const objectStore = this._db.createObjectStore(
          environment.expensesObjStoreName,
          { keyPath: 'id' },
        );
  ...
  async writeToStore<T>(store: string, data: T[]): Promise<void> {
    ...
    transaction.onerror = (event) => console.error('a transaction error occurred: ', event);
  ...
  async deleteFromStore(store: string, key: string): Promise<void> {
    await this.ready;
    const req = this._db.transaction(store, 'readwrite').objectStore(store).delete(key);
  ...
  async bulkReadFromStore<T>(store: string, callbackFn?: (data: T[]) => void): Promise<void> {
    await this.ready;
